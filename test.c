...
/*   Updated: 2025/08/06 16:25:17 by alerusso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_txtr(t_texture *txtr, char *name, void *connect, int size[2])
{
	...
	if (!txtr->ptr)
		return ;
	mlx_get_data_addr(txtr->ptr, &txtr->bpp, &txtr->size[X], &txtr->endian);
	...
	close(fd);
	height = sub_strlen(line, " ", EXCLUDE);
	height += 1;
	printf("%s\n", height);
	txtr->size[Y] = ft_atoi(height) * txtr->bpp;
	free(line);
}
